...
//                }
                ...
                if ($result === false) {
                    ...
                    if ($result['extra'][0]['field'] == "price") {
                        $bmsg = "خطای مقدار قیمت | ";
                    } else {
                        $bmsg = "";
                    }
                    $Message = $bmsg . $result['extra'][0]['error'];
                    print_r($result);
                    $Fault = '';
                }
 ...
 */
add_action('woocommerce_admin_order_data_after_billing_address', 'field_display_admin_order_meta_sama', 10, 1);
function field_display_admin_order_meta_sama($order)
{
    $order_id = method_exists($order, 'get_id') ? $order->get_id() : $order->id;
    $payment_type_order_info_status = true;
    if (get_post_meta($order_id, 'sama_payment_type_label', true) == "pardakht_etebari") {
        $payment_type_order_info = "پرداخت اعتباری";
    } elseif (get_post_meta($order_id, 'sama_payment_type_label', true) == "pardakht_zemanati") {
        $payment_type_order_info = "پرداخت ضمانتی";
    } else {
        $payment_type_order_info_status = false;
    }
    if ($payment_type_order_info_status == true) {
        echo '<style>
        .payment_sama_type_display {
            background: linear-gradient(352deg, #34e3ff, #20c5a2);
    color: #fff;
    font-size: 1rem;
    text-align: center;
    padding: 0.7rem;
    border: 1px solid #8bf7ff;
    border-radius: 9px;
    box-shadow: 0 0 9px #8e8e8e55;
        }
    </style>';
        echo '<div class="payment_sama_type_display">نوع پرداخت سما :  <strong>' . $payment_type_order_info . '</strong></div>';
    }
}
